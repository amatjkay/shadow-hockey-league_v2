...
class TestAuditService:
    ...
    def test_log_action_login_success(self, app, admin_user):
        ...
        with app.app_context():
            audit_entry = log_action(user_id=admin_user.id, action="LOGIN")
    ...
    def test_log_action_flush_cache_success(self, app, admin_user):
        ...
        with app.app_context():
            audit_entry = log_action(user_id=admin_user.id, action="FLUSH_CACHE")
    ...
    def test_log_action_invalid_user_id(self, app):
        ...
        with app.app_context():
            with pytest.raises(ValueError, match="user_id is required"):
                log_action(user_id=0, action="CREATE")
    ...
    def test_log_action_invalid_action(self, app, admin_user):
        ...
        with app.app_context():
            ...
            with pytest.raises(ValueError, match="action is required"):
                log_action(user_id=admin_user.id, action="")

            # Too long action
            with pytest.raises(ValueError, match="action is required and must be <= 50 characters"):
                log_action(user_id=admin_user.id, action="A" * 51)
    ...
    def test_log_action_invalid_target_model(self, app, admin_user):
        ...
        with app.app_context():
            with pytest.raises(ValueError, match="target_model must be <= 50 characters"):
                log_action(user_id=admin_user.id, action="CREATE", target_model="M" * 51)

    def test_log_action_database_error(self, app, admin_user):
        """Test log_action handling of database errors."""
        with app.app_context():
            with patch(
                "services.audit_service.db.session.commit", side_effect=Exception("DB Error")
            ):
                audit_entry = log_action(user_id=admin_user.id, action="CREATE")
    ...
    def test_cleanup_old_audit_logs_database_error(self, app):
        ...
        with app.app_context():
            ...
            with patch(
                "services.audit_service.db.session.commit", side_effect=SQLAlchemyError("DB Error")
            ):
...
def sample_audit_logs(app, admin_user):
    ...
    with app.app_context():
        ...
        # DELETE action
        log = AuditLog(
            user_id=admin_user.id, action="DELETE", target_model="Achievement", target_id=789
        )
        db.session.add(log)
        logs.append(log)

        # LOGIN action
        log = AuditLog(user_id=admin_user.id, action="LOGIN")
        db.session.add(log)
        logs.append(log)
