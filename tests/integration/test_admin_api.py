...
def reference_data(db_session):
    ...
    season_old = Season(code='22/23', name='Season 22/23', multiplier=0.85, start_year=2022, end_year=2023, is_active=True)
...
class TestBulkCreateAchievements:
    ...
    def test_bulk_create_skips_inactive(self, client, admin_user, manager_data, reference_data):
        ...
        data = response.get_json()
        assert data['summary']['created'] == 1
        assert data['summary']['skipped_duplicates'] == 1  # The inactive one is skipped as duplicate? No, it should be skipped as inactive
    ...
    def test_bulk_create_requires_create_permission(self, client, moderator_user, manager_data, reference_data):
