...
from models import Achievement, AchievementType, League, Manager, Season, db
...
def run_audit():
    ...
    with app.app_context():
        ...
        ach_invalid_type = Achievement.query.filter(
            ~Achievement.type_id.in_(db.session.query(AchievementType.id))
        ).count()
        ach_invalid_league = Achievement.query.filter(
            ~Achievement.league_id.in_(db.session.query(League.id))
        ).count()
        ach_invalid_season = Achievement.query.filter(
            ~Achievement.season_id.in_(db.session.query(Season.id))
        ).count()
        ...
        for ach in Achievement.query.limit(100).all():
            ...
            if round(ach.final_points) != expected:
                ...
                if mismatch_count <= 5:
                    print(
                        f"  ! Mismatch in ACH {ach.id}: DB={ach.final_points}, Expected={expected}"
                    )
