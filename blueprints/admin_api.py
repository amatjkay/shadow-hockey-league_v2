...
def get_achievement_points(type_id: int) -> Any:
    ...
    try:
        ...
        # Determine base points based on league hierarchy (parent_code-aware).
...
def calculate_points() -> Any:
    ...
    try:
        ...
        # Calculate base points based on league hierarchy (parent_code-aware).
        base_points = get_base_points(ach_type, league)
        points_source = league.base_points_field
...
def create_manager_achievement(manager_id):
    ...
    try:
        ...
        if league.code in ("2.1", "2.2") and season.start_year and season.start_year < 2025:
            return (
                jsonify({"error": f"League {league.code} is only available from season 25/26"}),
                400,
            )

        # V-001: Duplicate check
        existing = Achievement.query.filter_by(
            manager_id=manager_id, type_id=type_id, league_id=league_id, season_id=season_id
        ).first()
        if existing:
...
def update_manager_achievement(manager_id, achievement_id):
    ...
    try:
        ...
        achievement = (
            db.session.query(Achievement)
            .filter_by(id=achievement_id, manager_id=manager_id)
            .first()
        )
        ...
        if duplicate:
            return (
                jsonify({"error": "Duplicate achievement exists", "existing_id": duplicate.id}),
                409,
            )
        ...
        root_code = league.parent_code or league.code
        achievement.base_points = (
            ach_type.base_points_l1 if root_code == "1" else ach_type.base_points_l2
        )
        ...
        db.session.rollback()
        api_logger.error(
            f"Error in PUT /admin/api/managers/{manager_id}/achievements/{achievement_id}: {e}"
        )
        return jsonify({"error": "Internal server error"}), 500


@admin_api_bp.route(
    "/managers/<int:manager_id>/achievements/<int:achievement_id>", methods=["DELETE"]
)
@admin_required
def delete_manager_achievement(manager_id, achievement_id):
    ...
    try:
        ...
        db.session.rollback()
        api_logger.error(
            f"Error in DELETE /admin/api/managers/{manager_id}/achievements/{achievement_id}: {e}"
        )
        return jsonify({"error": "Internal server error"}), 500
