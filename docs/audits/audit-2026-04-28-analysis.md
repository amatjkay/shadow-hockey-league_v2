...
1. **Linear MCP:** установить нативную интеграцию `linear` или MCP-сервер `linear`? **Status:** ✅ done — Linear MCP подключён, все задачи T-L-1..T-L-6 выполнены (commit `7758bd6`).
2. **PR #11:** **Status:** ✅ done — закрыт без мерджа (контент уже в `main` через #25). См. комментарий в PR #11.
3. **Стек `devin/integration-analyst-fixes` (#15/#16/#17):**
   - **PR #15** — **Status:** ✅ done — закрыт (dev-cleanup, орфан-база, без срочности; при необходимости переоткрыть стандалоном на `main`).
...
- **T-L-1** | Cancel TIK-12 (premature optimization). **Зависимости:** Linear MCP. **Готовность:** статус Cancelled, комментарий со ссылкой на `services/cache_service.py::@cache.memoize`. **Status:** ✅ done — [TIK-12 → Canceled](https://linear.app/tikispace/issue/TIK-12/implement-pre-calculated-manager-ratings).
- **T-L-2** | Cancel TIK-18 (no scope), TIK-19 (covered by tests). **Зависимости:** Linear MCP. **Готовность:** оба Cancelled. **Status:** ✅ done — [TIK-18 → Canceled](https://linear.app/tikispace/issue/TIK-18/formalize-tandem-data-structure), [TIK-19 → Canceled](https://linear.app/tikispace/issue/TIK-19/point-calculation-validation-script).
- **T-L-3** | Mark TIK-16 as Done (выполнено в TIK-33 / PR #24). **Зависимости:** Linear MCP. **Готовность:** статус Done, ссылка на PR #24. **Status:** ✅ done — [TIK-16 → Done](https://linear.app/tikispace/issue/TIK-16/update-project-knowledgemd-with-rating-baselines).
- **T-L-4** | Создать новый тикет [TIK-36](https://linear.app/tikispace/issue/TIK-36/b9-audit-log-not-written-in-production-set-current-user-for-audit): B9 — Audit log not wired in production. Priority: P1. Включить ссылки на `services/audit_service.py:209-216`, `:286`, и план T-B9-1/2/3. **Зависимости:** Linear MCP. **Готовность:** тикет открыт, P1, in `Backlog`. **Status:** ✅ done — TIK-36 created (P1, labels: security/compliance/Bug).
- **T-L-5** | Создать тикет [TIK-37](https://linear.app/tikispace/issue/TIK-37/b10-health-endpoint-blocks-5-7s-when-redis-unavailable-missing-socket): B10 — `/health` Redis blocking без timeout. Priority: P2 (dev), P3 (prod). Ссылка на `blueprints/health.py:80`. **Зависимости:** Linear MCP. **Готовность:** тикет открыт. **Status:** ✅ done — TIK-37 created (P3, labels: Bug/performance).
- **T-L-6** | Создать тикет [TIK-38](https://linear.app/tikispace/issue/TIK-38/b11-app-startup-banner-advertises-http-requests-total-but-metrics): B11 — Metrics banner mismatch. Priority: P3. Ссылка на `app.py:235`. **Зависимости:** Linear MCP, T-V-3. **Готовность:** тикет открыт. **Status:** ✅ done — TIK-38 created (P4 — bumped down to Low per audit doc, labels: Bug/observability).
