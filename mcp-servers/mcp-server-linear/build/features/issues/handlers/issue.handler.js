...
export class IssueHandler extends BaseHandler {
    ...
    async handleCreateIssues(args) {
        try {
            ...
            return this.createResponse(`Successfully created ${createdIssues.length} issues:\n` +
                createdIssues
     ...
     */
    async handleBulkUpdateIssues(args) {
        try {
            const client = this.verifyAuth();
    ...
    async handleSearchIssues(args) {
        try {
            ...
            const result = (await client.searchIssues(filter, args.first || 50, args.after, args.orderBy || "updatedAt"));
     ...
     */
    async handleSearchIssuesByIdentifier(args) {
        try {
            const client = this.verifyAuth();
            ...
            const result = (await client.searchIssues({ identifier: { in: args.identifiers } }, 100, undefined, "updatedAt"));
            return this.createJsonResponse(result);
        }
        catch (error) {
    ...
    async handleGetIssue(args) {
        try {
            ...
            const result = (await client.searchIssues({ identifier: { in: [args.identifier] } }, 1, undefined, "updatedAt"));
    ...
    async handleEditIssue(args) {
        try {
            ...
            if (Object.keys(updateInput).length === 0) {
                throw new Error("No fields provided to update for issue " + args.issueId);
            }
            ...
            if (!result?.issueUpdate?.success || !result?.issueUpdate?.issue) {
                throw new Error(`Failed to update issue ${args.issueId}. API response: ${JSON.stringify(result)}`);
            }
