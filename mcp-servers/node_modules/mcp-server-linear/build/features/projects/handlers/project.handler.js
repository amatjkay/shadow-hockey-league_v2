...
export class ProjectHandler extends BaseHandler {
    ...
    async handleCreateProjectWithIssues(args) {
        try {
            ...
            if (!args.project.teamIds ||
                !Array.isArray(args.project.teamIds) ||
                args.project.teamIds.length === 0) {
                throw new Error("Project requires teamIds as an array with at least one team ID.\n" +
                    "Example:\n" +
                    "{\n" +
                    "  project: {\n" +
                    '    name: "Project Name",\n' +
            ...
            if (!Array.isArray(args.issues)) {
                throw new Error("Issues parameter must be an array of issue objects.\n" +
                    'Example: issues: [{ title: "Issue Title", teamId: "team-id-1" }]');
            }
            // Validate each issue has required teamId
            args.issues.forEach((issue, index) => {
                if (!issue.teamId) {
                    throw new Error(`Issue at index ${index} is missing required teamId.\n` +
     ...
     */
    async handleGetProjectMilestones(args) {
        try {
            const client = this.verifyAuth();
            this.validateRequiredParams(args, ["projectId"]);
            const result = await client.getProjectMilestones(args.projectId, args.filter, args.first, args.after, args.last, args.before, args.includeArchived, args.orderBy);
            return this.createJsonResponse(result);
        }
        catch (error) {
     ...
     */
    async handleCreateProjectMilestone(args) {
        try {
            const client = this.verifyAuth();
     ...
     */
    async handleUpdateProjectMilestone(args) {
        try {
            const client = this.verifyAuth();
            this.validateRequiredParams(args, ["id"]);
     ...
     */
    async handleDeleteProjectMilestone(args) {
        try {
            const client = this.verifyAuth();
            this.validateRequiredParams(args, ["id"]);
