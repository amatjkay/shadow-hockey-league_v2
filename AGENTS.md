...
| `karpathy-guidelines` | Behavioral guardrails: think first, simplicity, surgical changes, goal-driven execution | writing, reviewing, or refactoring code — complements § 5 coding standards |
| `task-formulation` | Checklist that turns a vague request into a well-formed task (Context / Result / DoD / Anti-Goals) | before any non-trivial code change — gates entry to `architect`/`coder`; rendered as `.github/ISSUE_TEMPLATE/well-formed-task.md` |
...
| 2026-05-07 | Integrate Karpathy behavioral guidelines (from `forrestchang/andrej-karpathy-skills`): added `CLAUDE.md` (Claude Code), `.cursor/rules/karpathy-guidelines.mdc` (Cursor IDE), `.agents/skills/karpathy-guidelines/SKILL.md` (project skill). Updated § 3 skills table. Four principles: Think Before Coding, Simplicity First, Surgical Changes, Goal-Driven Execution. Complements existing § 5 coding standards; project rules win on conflict. No source-code, test, or schema changes. | AI |
| 2026-05-11 | Added `task-formulation` skill (`.agents/skills/task-formulation/SKILL.md`) — 4-section checklist (Context / Result shape / Definition of Done / Scope & Anti-Goals) that gates non-trivial work into the `architect → coder → reviewer` pipeline. Rendered as `.github/ISSUE_TEMPLATE/well-formed-task.md`; `docs/AI_WORKFLOW.md` shortened to a pointer. Added `docs/wiki/` — thin Obsidian-compatible navigation vault with 5–15-line section notes that cross-link the canonical docs (`AGENTS.md`, `PROJECT_KNOWLEDGE.md`, `docs/ARCHITECTURE.md`, `docs/API.md`, source files) without duplicating their content; `docs/INDEX.md` updated with both entry points. `.gitignore` excludes `docs/wiki/.obsidian/`. No source-code, test, or schema changes. | AI |
| 2026-05-11 | TIK-86 — `services/rating_service.py` gains module-level `_LEADERBOARD_LOCK` that serializes `build_leaderboard()` to dodge a SQLAlchemy 2.0 cython result-processor race under concurrent joinedload reads (manifested as `IndexError: tuple index out of range`). Production `gunicorn --workers 4 --sync` never contends the lock; the lock matters for the threaded test client + any future `--threads >1` worker. Regression test: `tests/integration/test_routes.py::TestConcurrentRequests::test_concurrent_homepage_requests_stress` (20×10 fan-out, >99% catch rate). | AI |
| 2026-05-12 | Docs cleanup: removed 8 stale / empty / pointer files (`MCP_LINEAR_SETUP.md`, `docs/MCP_SETUP.md`, `docs/API_EN.md`, `docs/comprehensive_testing_plan.md`, `test-plan.md`, `docs/MIGRATIONS.md`, `docs/GITHUB_CLI.md`, `docs/AI_WORKFLOW.md`); archived one-off `docs/handoff-2026-05-07.md` to `docs/archive/`. Updated stale test count (472 → 561) and rescaled point values (compact-10, TIK-80: TOP1 L1=10.00 / L2=6.00 etc., season multipliers `0.7 ^ years_ago`) in `README.md`, `PROJECT_KNOWLEDGE.md`, `docs/projectbrief.md`, `docs/activeContext.md`, `docs/techContext.md`, `docs/INDEX.md`, `docs/wiki/Testing and QA.md`, `docs/wiki/Caching.md`, `.agents/skills/verification/SKILL.md`. `docs/activeContext.md` rewritten to reflect post-TIK-86 maintenance state. No source-code, test, or schema changes. | AI |
| 2026-05-13 | **TIK-88 — owner-actions catalog + secrets fail-fast + .gitignore.** Added `docs/owner-actions.md` as the single source of truth for owner-actionable follow-ups (T01..T14, mirrored to TIK-88 / TIK-89 / TIK-90 in Linear); replaces the inline list in `docs/activeContext.md` § Immediate Next Steps. `config.py` gains `validate_production_secrets()`; `app.py::create_app()` calls it when `FLASK_ENV=production` and `RuntimeError`s out on dev-default `SECRET_KEY` / `WTF_CSRF_SECRET_KEY` / `API_KEY_SECRET`. `.gitignore` now excludes `bandit_report*.json`, `.coverage.*`, `*.log`, `.tool-versions`; the tracked-but-empty `bandit_report.json` + `bandit_report_main.json` are removed. Regression coverage: `tests/test_config_fail_fast.py` (8 tests). `docs/INDEX.md` registers the new catalog. No schema changes. | AI |
